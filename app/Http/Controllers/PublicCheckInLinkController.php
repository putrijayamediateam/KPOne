...
class PublicCheckInLinkController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $links = PublicCheckInLink::query()
            ...
            ->map(fn (PublicCheckInLink $link): array => [
                ...
                'label' => $link->label,
                'status' => $link->is_active ? 'active' : 'revoked',
    ...
    public function store(Request $request, PublicCheckInLinkService $service): JsonResponse
    {
        ...
        return response()->json(['issuedLink' => $this->issuedLink($issued['link'], $issued['rawToken'])], 201);
    }

    public function rotate(Request $request, string $publicCheckInLink, PublicCheckInLinkService $service): JsonResponse
    {
        $link = $this->scopedLink($request, $publicCheckInLink);
        $issued = $service->rotate($request->user(), $link);

        return response()->json(['issuedLink' => $this->issuedLink($issued['link'], $issued['rawToken'])]);
    }
    ...
    /** @return array{publicId: string, url: string} */
    private function issuedLink(PublicCheckInLink $link, string $rawToken): array
    {
        return [
            'publicId' => $link->public_id,
