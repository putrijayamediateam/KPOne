...
class PostgresClinicalSafetyRegressionTest extends TestCase
{
    ...
    private function waitReady(array $workers): void
    {
        ...
        throw new RuntimeException('Clinical safety worker did not report ready.');
    }
    ...
    private function waitForOutput(Process $process, string $needle): void
    {
        ...
        throw new RuntimeException('Clinical safety worker protocol timeout.');
    }
    ...
    private function waitForDatabaseBlock(Process $process, ?int $expectedBlockerPid = null): void
    {
        ...
        $pid = (int) ($matches[1] ?? 0);
        if ($pid <= 0) {
            throw new RuntimeException('Clinical safety worker did not report a valid PostgreSQL backend PID.');
        }
        $deadline = microtime(true) + 10;
        do {
            if ($process->isTerminated()) {
                throw new RuntimeException('Clinical safety worker exited before PostgreSQL blocking was observed.');
            }
        ...
        throw new RuntimeException('Expected PostgreSQL Clinical safety blocking was not observed.');
    }
